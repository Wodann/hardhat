...
function classifyScenario(id, resolution, expected) {
  ...
  if (resolution.stamp !== undefined) {
    ...
    // is the correct resolution. A *defined* stamp never takes this path: it
    // proves a locally published tarball, which for any stamp but this run's
    // is stale. Caveat: a stale unstamped tarball published by a pre-stamping
    // run is indistinguishable from the registry release and would be
    // reported as OK.
    return {
      ok: true,
