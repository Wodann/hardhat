 ...
 * run, timing each step with the high-resolution monotonic clock. Steps share
 * state across the sequence, so the cost of resetting between samples is paid
 * once per run by a `measure: false` reset step instead of by a hyperfine
 * `prepare` before every timed run. Returns one entry per measured step.
 */
